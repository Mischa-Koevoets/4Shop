	...
	<table class="table table-striped table-hover">
		...
		@foreach($orders as $order)
			<tr>
				...
				<td>
                    <form action="{{ route('admin.orders.toggle', $order) }}" method="post">
                        @csrf
                        @method('patch')
                        <button type="submit">
                        @if($order->delivered)
                            <span class="badge badge-success">Afgeleverd</span>
                        @else
                            <span class="badge badge-warning">niet afgeleverd</span>
                        @endif
                        </button>
                    </form>
                </td>
